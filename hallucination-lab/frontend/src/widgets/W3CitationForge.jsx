...
import { extractCitations } from '../lib/utils.js'
...
function buildSortItems(modelText) {
  // Both piles carry a full reporter citation. Showing the model's cases as
  // bare party names next to fully cited real ones made the fakes obvious from
  // formatting alone, which is the opposite of what this module claims.
  const fabricated = extractCitations(modelText).slice(0, 3)
    .map((c) => ({ name: c.full, searchName: c.name, isReal: false }))
  // Match the real pile to the fabricated pile so the counts stay even; an
  // uneven split is itself a tell.
  const real = [...REAL_CASES].sort(() => Math.random() - 0.5).slice(0, fabricated.length)
    .map((c) => ({ name: `${c.name}, ${c.cite}`, searchName: c.name, isReal: true, holding: c.holding }))
      ...
      {text && (
        <>
          ...
          <div className="card">
            ...
            <p>
              The {items.length} rows below come from two places: the{' '}
              <strong>cases the Lab Model just wrote above</strong>, shuffled with an equal number of{' '}
              <strong>real cases the lab supplied</strong> (actual U.S. Supreme Court decisions).
              Every row carries a full citation, because that is the point: a fabricated citation is
              formatted exactly like a real one. Mark each Real or Fake, then check.
              Score to beat: {items.length} of {items.length}.
            </p>
            {items.map((it, i) => {
              ...
              return (
                <div key={i} className="sort-item" data-verdict={verdict}>
                  <span style={{ flex: 1 }}>
                    ...
                    {' '}<a href={scholarUrl(it.searchName || it.name.split(',')[0])} target="_blank" rel="noreferrer">verify</a>
                  </span>
